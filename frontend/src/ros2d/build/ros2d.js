...
const createjs = require("@createjs/easeljs/src/main")
 ...
 */
const OccupancyGrid = function OccupancyGrid(options) {
    options = options || {};
    var message = options.message;
    ...
    // create the bitmap
    // createjs.Bitmap.initialize(canvas);
    const bm = new createjs.Bitmap(canvas)
    bm.draw(context)
    // change Y direction
 ...
 */
const OccupancyGridClient = (function (EventEmitter2) {
    ...
    OccupancyGridClient.prototype.subscribe = function subscribe() {
        ...
        // this.rosTopic.subscribe(console.log)
    };
    OccupancyGridClient.prototype.processMessage = function processMessage(message) {
        console.log(message)
        console.log(message.data.reduce(function(prev, next, index, array) {
                return prev + next;
            })
        )
        // check for an old map
 ...
 */
const Grid = function Grid(options) {
    var that = this;
    options = options || {};
    var size = options.size || 10;
    ...
    graphics.endStroke();
    // create the shape
    createjs.Shape.initialize(graphics)
};
Grid.prototype.__proto__ = createjs.Shape.prototype;
 ...
 */
var NavigationArrow = function (options) {
    var that = this;
    options = options || {};
    var size = options.size || 10;
    var strokeSize = options.strokeSize || 3;
    ...
    // create the shape
    // createjs.Shape.call(this, graphics)
    const a = new createjs.Shape(graphics)
...
NavigationArrow.prototype.__proto__ = createjs.Shape.prototype;
 ...
 */
const Viewer = function Viewer(options) {
    var that = this;
    options = options || {};
...
export {Viewer, OccupancyGridClient, NavigationArrow}
